...
@interface FTGymViewForLaunchMatch () <UITableViewDelegate, UITableViewDataSource,UICollectionViewDelegate, UICollectionViewDataSource, FTCycleScrollViewDelegate,FTSelectCellDelegate>
...
@implementation FTGymViewForLaunchMatch
...
- (void) getTableViewDataFromWeb {
    ...
    [dic setObject:_pageSize forKey:@"pageSize"];//pageSize
    [dic setObject:[NSString stringWithFormat:@"%d", _pageNum] forKey:@"pageNum"];//pageNum
    NSString *ts = [NSString stringWithFormat:@"%.0f", [[NSDate date] timeIntervalSince1970]];
    ...
    NSLog(@"urlstring:%@",urlString);
    [NetWorking getGymsForArenaByDic:dic option:^(NSDictionary *dict) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell labelsViewAdapter:dic[@"sup_referee"]];
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    FTGymBean *bean = [FTGymBean new];
    [bean setValuesWithDic:newsDic];
    gymDetailViewController.gymBean = bean;
    
    if ([self.delegate respondsToSelector:@selector(pushToController:)]) {
        [self.delegate pushToController:gymDetailViewController];
    }
