...
@interface FTHeightPickerView () <UIPickerViewDataSource,UIPickerViewDelegate>
...
@implementation FTHeightPickerView
...
- (void) setSubviews {
    ...
    [_pickerView selectRow:125 inComponent:0 animated:YES];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view
{
    ...
    if (component==0) {
        ...
        label.text= [NSString stringWithFormat:@"%lu",row +50 ];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    //将选择器结果显示到label
    _resultLabel.text = [NSString stringWithFormat:@"%ldcm",selectH];
                          ...
                          if (dict != nil) {
                              ...
                              if (status == true) {
                                  ...
                                  if ([self.delegate respondsToSelector:@selector(didSelectedDate: type:)]) {
                                      [self.delegate didSelectedDate:propertValue type:FTPickerTypeHeight];
                                  }
                                  ...
                                  if ([self.delegate respondsToSelector:@selector(didSelectedDate: type:)]) {
                                      [self.delegate didSelectedDate:nil type:FTPickerTypeHeight];
                                  }
                                  
                              }
                          }else {
                              
                              [[UIApplication sharedApplication].keyWindow showHUDWithMessage:@" 身高修改失败，请稍后再试"];
